...
GPUdii() void GPUTPCCFNoiseSuppression::Thread<GPUTPCCFNoiseSuppression::noiseSuppression>(int nBlocks, int nThreads, int iBlock, int iThread, GPUSharedMemory& smem, processorType& clusterer)
{
  ...
  noiseSuppressionImpl(get_num_groups(0), get_local_size(0), get_group_id(0), get_local_id(0), smem, *clusterer.getCalibration(), chargeMap, isPeakMap, clusterer.mPpeakPositions, clusterer.mPmemory->counters.nPeaks, clusterer.mPisPeak);
}
  ...
  ulong* peaks)
{
  ...
  *peaks = 0;
  ...
  findMinima(
    ...
    16,
    q,
    calibration.noiseSuppressionEpsilon,
    minimas,
    bigger);
