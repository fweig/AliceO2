...
GPUdii() void GPUTPCCFPeakFinder::Thread<0>(int nBlocks, int nThreads, int iBlock, int iThread, GPUSharedMemory& smem, processorType& clusterer)
{
  ...
  findPeaksImpl(get_num_groups(0), get_local_size(0), get_group_id(0), get_local_id(0), smem, chargeMap, clusterer.mPpositions, clusterer.mPmemory->counters.nPositions, *clusterer.getCalibration(), clusterer.mPisPeak, isPeakMap);
}
...
GPUdii() bool GPUTPCCFPeakFinder::isPeak(
  ...
  const TPCCFCalibration& calibration,
  ChargePos* posBcast,
  PackedCharge* buf)
{
  ushort ll = get_local_id(0);

  bool belowThreshold = (q <= calibration.qmaxCutoff);
                                              ...
                                              Array2D<uchar>& peakMap)
{
  ...
  uchar peak;
  peak = isPeak(smem, charge, pos, SCRATCH_PAD_SEARCH_N, chargeMap, calibration, smem.posBcast, smem.buf);
  ...
  peakMap[pos] = (uchar(charge > calibration.extendInnerChargeThreshold) << 1) | peak;
}
