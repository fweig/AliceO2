...
GPUdii() void GPUTPCCFClusterizer::Thread<0>(int nBlocks, int nThreads, int iBlock, int iThread, GPUSharedMemory& smem, processorType& clusterer, char onlyMC)
{
  ...
  GPUTPCCFClusterizer::computeClustersImpl(get_num_groups(0), get_local_size(0), get_group_id(0), get_local_id(0), clusterer.mPmemory->fragment, smem, chargeMap, clusterer.mPfilteredPeakPositions, *clusterer.getCalibration(), CPU_PTR(&labelAcc), clusterer.mPmemory->counters.nClusters, clusterer.mNMaxClusterPerRow, clusterer.mPclusterInRow, clusterOut, clusterer.mPclusterPosInRow);
}
                                                       ...
                                                       uint* clusterPosInRow)
{
  ...
  pc.toNative(pos, charge, calib.minSplitChargeNum, myCluster);

  bool aboveQTotCutoff = (myCluster.qTot > calib.qtotCutoff);
  ...
  uchar* innerAboveThreshold)
{
  ...
  for (ushort i = 0; i < N; i++) {
    ...
    aboveThreshold |= (uchar(q > calib.extendInnerChargeThreshold) << i);
  }
